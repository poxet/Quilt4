...
namespace Quilt4.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            string username;
            if (!_accountRepository.GetUsers().Any(x => x.UserName == model.User))
            {
                //username = _accountRepository.GetUsers().Any(x => x.UserName == model.User) ? _accountRepository.GetUsers().Single(x => x.UserName == model.User).UserName : model.User;
                if (_accountRepository.GetUsers().Any(x => x.Email == model.User))
                {
                    username = _accountRepository.GetUsers().Single(x => x.Email == model.User).UserName;
                }
                else
                {
                    username = model.User;
            ...
            var result = await _accountRepository.PasswordSignInAsync(username, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
